...
function listActionUrl(section, metadata) {
    return `/admin/${formatting_1.parseName(section.name)}/${formatting_1.parseName(metadata.name)}/action`;
}
...
function changeActionUrl(section, metadata, entity) {
    const primaryKey = urlEncodePrimaryKey(entity_1.getPrimaryKeyValue(metadata, entity));
    return `/admin/${formatting_1.parseName(section.name)}/${formatting_1.parseName(metadata.name)}/${primaryKey}/action`;
}
